...
import { depurarLineasPlanificacionBC } from '@/app/actions/business-central';

export default function DepurarPlanificacionBoton() {
  const [jobNo, setJobNo] = useState('E260034');
  const [jobTaskNo, setJobTaskNo] = useState('95.01');
  ...
  async function handleDepurar() {
    ...
    const r = await depurarLineasPlanificacionBC(jobNo, jobTaskNo);
  ...
  return (
    <div className="mb-6 max-w-2xl">
      <div className="flex items-center gap-2 mb-2">
        <input className="input py-1 w-32" value={jobNo} onChange={(e) => setJobNo(e.target.value)} placeholder="Nº obra" />
        <input
          className="input py-1 w-24"
          value={jobTaskNo}
          onChange={(e) => setJobTaskNo(e.target.value)}
        ...
        <button onClick={handleDepurar} disabled={cargando} className="text-xs text-slate hover:text-grafito underline">
          {cargando ? 'Consultando BC…' : '🔍 Ver líneas de planificación crudas'}
        </button>
      </div>
